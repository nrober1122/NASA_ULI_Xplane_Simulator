...
def main():
    ...
    results_dir = scratch_dir / f"tiny_taxinet_attack_static_stride{stride}_{OBJECTIVE}"
    ...
    model = get_stride_network(stride=stride)
    model.eval()
    model.to(device)
    ...
    dset_train, dl_train = get_dataloader(pathlib.Path(DATA_DIR), stride, "train", dl_params)
    dset_test, dl_test = get_dataloader(pathlib.Path(DATA_DIR), stride, "validation", dl_params)
